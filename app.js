...
// ── STATE ────────────────────────────────────────────────────
let cart = [];
let currentCategory = 'all'; // Track active category for search filtering
...
const toastEl       = $('toast');
const searchInput   = $('searchInput'); // Search input element
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (splashOverlay) {
    ...
    }, 2500); // Fades out smoothly after 2.5 seconds
  }
});
...
function renderMenu(cat, searchQuery = '') {
  ...
  // 2. Filter by Search Query (if any text is typed)
  if (searchQuery.trim() !== '') {
  ...
  // 3. Handle Empty Search Results State cleanly
  if (items.length === 0) {
...
// Fixed minor syntax reference duplicate layout in internal tree
function cartDec(id) {
  const isDeal = String(id).startsWith('deal_');
  const c = cart.find(i => isDeal ? i.id === id : Number(i.id) === Number(id));
  if (!c) return;
...
// ── ADVANCED QUICK EXPRESS MULTI-CHANNEL ROUTING ENGINE ──
function handleQuickCheckout(method) {
  ...
  // For instant modes without forms
...
// Expose window references for inline triggers
